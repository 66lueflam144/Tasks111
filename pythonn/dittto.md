...
>在python文件里，一个.py文件称为一个模块。
- 使用模块可以避免函数名和变量名冲突。相同名字的函数和变量完全可以分别存在不同的模块里。
  
>为了避免模块名冲突，python引进了按目录来组织模块的方法，称为包（package）。

- 引入包以后，模块名称由`abc.py`变为`mypy.abc`。
- 每一个包目录下都必须有一个`__init__.py`文件（可以是空文件也可以有python代码——`__init__.py`本身就是一个模块，其名为包名。
- 存在多级目录，`mypy.web.scd`是一个模块，`mypy.web`也是一个模块。

---
##### **作用域**

- 正常函数和变量名是公开的，可以被直接引用。
- 类似`__xx__`是特殊变量，可以被直接引用，但有特殊用途。
- `_xxx`是非公开变量函数或变量，不能被直接引用。

---

#### **安装第三方模块**

- 安装第三方库`pip install xxx`，命令行中为`python -m pip install xxx`。
- `import xxx`直接引用已安装的第三方模块。


ps：<br>
- 默认情况下，python解释器会搜索当前目录、所有已安装的内置模块和第三方模块，搜索路径存放在模块的`path`变量中。
- 添加自己的搜索目录：
  - （1）直接修改`xxx.path`，添加所要搜索的目录：`import xxx`<br>`xxx.path.append('/path/to/file')`<br>该方法在运行时修改，运行结束后失效。
